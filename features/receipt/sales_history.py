...
class SalesHistoryMain(ctk.CTkFrame):
    def __init__(self, parent, main_app, user_role="employee"):
        ...
        # LEFT: Receipt List Container (fixed width)
        left_container = ctk.CTkFrame(main_content, fg_color="#ffffff", corner_radius=20, width=340)
        left_container.grid(row=0, column=0, sticky="ns", padx=(0, 60), pady=0)
        left_container.grid_propagate(False)
        left_container.grid_rowconfigure(0, weight=1)
        left_container.grid_columnconfigure(0, weight=1)
        self.receipt_list_frame = ctk.CTkScrollableFrame(left_container, fg_color="#ffffff", corner_radius=15, width=300)
        self.receipt_list_frame.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)
        ...
        self.header_frame = ctk.CTkFrame(self.right_panel, fg_color="transparent", width=300)
