...
class TicketPanel(ctk.CTkFrame):
    def __init__(self, master, on_split_popup, *args, **kwargs):
        ...
        # === Header ===
        header_frame = ctk.CTkFrame(self, fg_color="transparent", width=300)
        ...
        # === Subtotal ===
        subtotal_frame = ctk.CTkFrame(self, fg_color="transparent", width=260)
        subtotal_frame.grid(row=2, column=0, sticky="ew", padx=16, pady=(0, 10))
        ctk.CTkLabel(subtotal_frame, text="SubTotal", font=("Unbounded", 20), text_color="#4e2d18").pack(side="left",anchor="e")
        self.total_display = ctk.CTkLabel(subtotal_frame, text="₱ 0.00", font=("Poppins", 20, "bold"), text_color="#4d6443", width=100)
        self.total_display.pack(side="right")

        # === Tabs (Change / Discount) ===
        self.tab_frame = ctk.CTkFrame(self, fg_color="transparent", width=260)
        self.tab_frame.grid(row=3, column=0, sticky="ew", pady=(10, 5))

        self.change_tab_btn = ctk.CTkButton(self.tab_frame, text="Change", width=80,
                                            command=lambda: self.switch_tab(0), hover_color="#e8e4df",
                                            font=("Unbounded", 14), fg_color="#f2efea", text_color="#4e2d18")
        self.change_tab_btn.grid(row=0, column=0, padx=5)

        self.discount_tab_btn = ctk.CTkButton(self.tab_frame, text="Discount", width=80,
                                              command=lambda: self.switch_tab(1), hover_color="#e8e4df",
                                              font=("Unbounded", 14), fg_color="#f2efea", text_color="#4e2d18")
        self.discount_tab_btn.grid(row=0, column=1, padx=5)

        self.tab_content_container = ctk.CTkFrame(self, fg_color="transparent", width=260)
        self.tab_content_container.grid(row=4, column=0, sticky="ew", padx=10, pady=(0, 5))

        # === Change Panel ===
        self.change_frame = ctk.CTkFrame(self.tab_content_container, fg_color="transparent", width=260)
        ...
        for idx, (label, value, color) in enumerate(discount_buttons):
            ...
            btn = ctk.CTkButton(self.discount_frame, text=label, width=70, height=36,
                                font=("Unbounded", 13), text_color=color,
                                border_color=color, fg_color="#f2efea",
                                border_width=2, corner_radius=8, hover_color="#e8e4df",
                                command=cmd)
            btn.grid(row=0 if idx < 4 else 1, column=idx % 4, padx=2, pady=2)

        # === Move entry fields to row=1, column=4 and column=5 (next to buttons)
        self.peso_input = ctk.CTkEntry(self.discount_frame, placeholder_text="₱ Amount", width=70, height=36,
                                    font=("Inter", 13), text_color="#4e2d18", border_color="#f2efea")
        self.peso_input.grid(row=1, column=2, padx=2, pady=2)

        self.percent_input = ctk.CTkEntry(self.discount_frame, placeholder_text="% Amount", width=70,height=36,
                                        font=("Inter", 13), text_color="#4e2d18", border_color="#f2efea")
        self.percent_input.grid(row=1, column=3, padx=2, pady=2)

        # Show change_frame by default
        self.change_frame.tkraise()

        # === Cash and Change Display ===
        self.cash_change_frame = ctk.CTkFrame(self, fg_color="transparent", width=260)
        self.cash_change_frame.grid(row=5, column=0, sticky="ew", padx=12, pady=(5, 5))

        cash_column = ctk.CTkFrame(self.cash_change_frame, fg_color="transparent", width=120)
        ...
        change_column = ctk.CTkFrame(self.cash_change_frame, fg_color="transparent", width=120)
        ...
        # === Payment Buttons ===
        self.payment_buttons_frame = ctk.CTkFrame(self, fg_color="transparent", width=260)
        self.payment_buttons_frame.grid(row=6, column=0, sticky="ew", padx=6, pady=(15, 10))

        self.gcash_btn = ctk.CTkButton(self.payment_buttons_frame, text="GCash", width=60, height=36,
                                       font=("Unbounded", 16, "bold"), fg_color="#0091f7", text_color="#f2efea", hover_color="#005ea8",
                                       corner_radius=8)
        self.gcash_btn.grid(row=0, column=0, padx=(0, 5))

        self.maya_btn = ctk.CTkButton(self.payment_buttons_frame, text="Maya", width=50, height=36,
                                      font=("Unbounded", 16, "bold"), fg_color="#9aca1f", text_color="#f2efea", hover_color="#7a9e18",
                                      corner_radius=8)
        self.maya_btn.grid(row=0, column=1, padx=(0, 5))

        self.split_btn = ctk.CTkButton(self.payment_buttons_frame, text="Split", width=60, height=36,
                                       font=("Unbounded", 16, "bold"), fg_color="#708a2e", text_color="#f2efea", hover_color="#5c7324",
                                       corner_radius=8,
                                       command=self.open_split_popup)
        self.split_btn.grid(row=0, column=2, padx=(0, 5))

        self.charge_btn = ctk.CTkButton(self.payment_buttons_frame, text="Charge", width=100, height=36,
                                        font=("Unbounded", 16, "bold"), fg_color="#708a2e", text_color="#f2efea", hover_color="#5c7324",
                                        corner_radius=8)
